...
// --- ESTRUCTURA DE LA INFO PARA "MONITOREO Y CONTROL" ---
const MONITOREOYCONTROL_DATA = {
  id: "04 - 05",
  titulo: "Monitoreo y Control",
  subtitulo: "Monitoring & Controlling Focus Area",
  descripcion: "Medir el desempeño y aplicar cambios.",
  definicion: "Área de enfoque que rastrea, revisa e informa el progreso del proyecto. Ocurre en PARALELO con todas las demás áreas, de forma continua durante todo el proyecto.",
  puntosClave: [
    "Monitorear y Controlar el Trabajo: comparar real vs. planificado",
    "Indicadores líderes (leading): anticipar problemas antes de que ocurra",
    "Indicadores rezagados (lagging): medir resultados pasados del trabajo",
    "Earned Value Management (EVM): integra alcance, cronograma y costos",
  ],
  ...
  tip: {
    ...
    info: "Monitoreo ≠ Control. Monitorear es OBSERVAR; controlar es ACTUAR. Siempre conecta los datos de monitoreo con acciones correctivas concretas."
  }
};
...
export default function DetalleMonitoreoYControl() {
    ...
    <div style={{ minHeight: "100vh", background: BG_PAGE, color: "#fff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ padding: "32px 48px", maxWidth: "1000px", margin: "0 auto", opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(15px)", transition: "all 0.5s ease" }}>
        ...
        <div style={{ fontSize: "13px", color: PURPLE_ACCENT, marginBottom: "25px", fontWeight: 600 }}>
          ...
          <span style={{ color: "#fff" }}>{MONITOREOYCONTROL_DATA.titulo}</span>
        </div>
        ...
        >
          ...
          {activeTab === "Definición" && (
            <div>
              ...
              <p style={{ fontSize: "17px", lineHeight: "1.8", color: "#f3e8ff", margin: 0 }}>{MONITOREOYCONTROL_DATA.definicion}</p>
            </div>
          )}
          ...
          {activeTab === "Puntos Clave" && (
            <div>
              ...
              <ul style={{ padding: 0, margin: 0, listStyle: "none" }}>
                {MONITOREOYCONTROL_DATA.puntosClave.map((punto, i) => (
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
                {MONITOREOYCONTROL_DATA.salidasPrincipales.map((area, i) => (
          ...
          {activeTab === "Procesos" && (
            <div>
              <h3 style={{ color: PURPLE_ACCENT, fontSize: "12px", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "15px" }}>{MONITOREOYCONTROL_DATA.procesos.subtitulo}</h3>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                {MONITOREOYCONTROL_DATA.procesos.items.map((item, i) => (
          ...
          {activeTab === "Tip" && (
            <div style={{ borderLeft: `4px solid ${PURPLE_ACCENT}`, paddingLeft: "25px", background: "rgba(192, 132, 252, 0.05)", padding: "20px 25px", borderRadius: "0 12px 12px 0" }}>
              <h3 style={{ color: PURPLE_ACCENT, fontSize: "12px", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "10px" }}>{MONITOREOYCONTROL_DATA.tip.subtitulo}</h3>
              <p style={{ fontSize: "16px", lineHeight: "1.7", color: "#e9d5ff", fontStyle: "italic", margin: 0 }}>
                "{MONITOREOYCONTROL_DATA.tip.info}"
              </p>
            </div>
          )}
        </div>
