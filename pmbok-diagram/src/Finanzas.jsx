...
// --- ESTRUCTURA DE LA INFO PARA "FINANZAS" (DOMINIO 4) ---
const FINANZAS_DATA = {
  id: "04 - 07",
  titulo: "Finanzas",
  ...
  definicion: {
    ...
    info: "Dominio que incluye los procesos para planificar, estimar, presupuestar, financiar, gestionar y controlar los costos del proyecto para maximizar el valor por unidad de inversión."
  },
  puntosClave: [
    "Estimación análoga: basada en proyectos similares históricos (macro, rápida)",
    "Estimación paramétrica: modelos matemáticos + datos históricos",
    "Estimación bottom-up: agregar estimados detallados de cada tarea",
    "Técnica PERT: (Opt + 4·Más probable + Pesimista) / 6",
    ...
    {/* ── TOP NAV BAR ── */}
      ...
      <div style={{ padding: "32px 48px", maxWidth: "1000px", margin: "0 auto", opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(15px)", transition: "all 0.5s ease" }}>
        ...
        <div style={{ fontSize: "13px", color: GREEN_ACCENT, marginBottom: "25px", }}>
          ...
          <span style={{ color: "#fff", fontWeight: 600}}>{FINANZAS_DATA.titulo}</span>
        </div>
        ...
        >
          ...
          {activeTab === "Definición" && (
            <div>
              <h3 style={{ color: GREEN_ACCENT, fontSize: "12px", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "15px" }}>{FINANZAS_DATA.definicion.subtitulo}</h3>
              <p style={{ fontSize: "18px", lineHeight: "1.8", color: "#ecfdf5", margin: 0 }}>{FINANZAS_DATA.definicion.info}</p>
            </div>
          )}
          ...
          {activeTab === "Puntos Clave" && (
            <div>
              ...
              <ul style={{ padding: 0, margin: "0 0 30px 0", listStyle: "none" }}>
                {FINANZAS_DATA.puntosClave.map((punto, i) => (
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
                {FINANZAS_DATA.areasEnfoque.map((area, i) => (
        ...
        {activeTab === "Procesos" && (
          ...
          {activeTab === "Tip" && (
            <div style={{ borderLeft: `4px solid ${GREEN_ACCENT}`, paddingLeft: "25px" }}>
              <h3 style={{ color: GREEN_ACCENT, fontSize: "12px", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "10px" }}>{FINANZAS_DATA.tip.subtitulo}</h3>
              <p style={{ fontSize: "17px", lineHeight: "1.7", color: "#d1fae5", fontStyle: "italic", margin: 0 }}>
                "{FINANZAS_DATA.tip.info}"
              </p>
            </div>
          )}
        </div>
